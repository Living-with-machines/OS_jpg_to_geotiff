...
# createDir('/mnt/ramdisk/OS_tiffs_cut/')
...
def extractMap(path):
    ...
    if __name__ == "__main__":
        ...
        # imgTk = ImageTk.PhotoImage(Image.open('./OS_tiffs/' + path)) --- no need to open the same image again, just use "img" instead:
        ...
        def printcoords(event):
            ...
            if [cx, cy] not in corners_pixels:
                ...
                if len(corners_pixels) > 3:
                    ...
                    directory = (
                        "/mnt/ramdisk/OS_tiffs_cut/"
                        + str(os.path.dirname(path))
                        + "/"
                    )
                    createDir(directory)
                    rgb.save(
                        "/mnt/ramdisk/OS_tiffs_cut/" + path,
                        "TIFF",
                        resolution=100.0,
                    )
                    root.destroy()
...
def addGcps(path, gcps):
    src = "/mnt/ramdisk/OS_tiffs_cut" + path
    ...
    if not path.startswith("."):
        ...
        for f in OS_coords["features"]:
            if (
                ...
                # print("going to extractMap")
                extractMap(path)
                # print("going to createCornerLatLng")
                corners = createCornerLatLng(sheet_ref, county)
                # print("going to createGcps")
                gcps = createGcps(corners)
                # print("going to addGcps")
                addGcps(path, gcps)
                print("... createGeoTiff")
                createGeoTiff(path)
                ...
                os.remove("/mnt/ramdisk/OS_tiffs_cut/" + path)
                break
...
# shutil.rmtree('/mnt/ramdisk/OS_tiffs_cut/')
